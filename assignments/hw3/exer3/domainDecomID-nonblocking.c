...
int main(int argc, char *argv[]){
	...
	//send ghostcell to previous
	MPI_Isend(&f[2], 1, MPI_DOUBLE, prev, 0, MPI_COMM_WORLD,&req[0]);
	//send ghostcell to next 
    	MPI_Isend(&f[nxn_loc-3], 1, MPI_DOUBLE, nxt, 1, MPI_COMM_WORLD,&req[2]);


    	MPI_Irecv(&f[0], 1, MPI_DOUBLE, prev, 1, MPI_COMM_WORLD,&req[3] );
    	MPI_Irecv(&f[nxn_loc-1], 1, MPI_DOUBLE, nxt, 0, MPI_COMM_WORLD, &req[1]);
